...
with open(csv_file_path, mode='r', newline='') as file:
    ...
    #read the cvs file as dictionary
    ...
    for row in csv_reader:
        ...
        #        #the draft email is the output of first task i.e persionalize_email task
...
crew = Crew(
    ...
    ## we normally created the task only need to unpack it .
    # we used * for unpack the element in the list
    #So, if personalize_email_tasks contains [task1, task2, task3] and ghostwrite_email_tasks contains [task4, task5], then [*personalize_email_tasks, *ghostwrite_email_tasks] would result in [task1, task2, task3, task4, task5].
    tasks=[
        *personalize_email_tasks,
        *ghostwrite_email_tasks
    ],
    ## while using the groq we can call 30 calls per minute which is good but the crew ai is fast so you might be croos the limit
    # so we set it 29
    max_rpm=29
)
